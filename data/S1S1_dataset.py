...
class S1S1Dataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        x_nor = normalization_S1S1(x,self.files[idx])
        #x_nor = x_nor.reshape((x_nor.shape[2], x_nor.shape[0], x_nor.shape[1]))
        x_nor = np.moveaxis(x_nor,-1,0)
        data = x_nor[:x_nor.shape[0]//2,:,:]
        target = x_nor[x_nor.shape[0]//2:,:,:]
...
def normalization_S1S1(input_array,file):
