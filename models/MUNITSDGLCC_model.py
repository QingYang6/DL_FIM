...
class MUNITSDGLCCModel(BaseModel):
    """
    ...
    def sample(self):
        ...
        if not self.opt.use_same_G:
            _, style = self.netG_B.encode(self.realB_C)
    ...
    def forward(self):
        ...
        if not self.opt.use_same_G:
            ...
            self.c_b, self.s_b_prime = self.netG_B.encode(self.realB_C)
            ...
            self.c_a_recon, self.s_b_recon = self.netG_B.encode(self.x_ab_cb)
    ...
    def backward_G(self):
        ...
        # GAN loss
        x_ba_a = torch.cat((self.x_ba, self.realB_C), 1)
        x_ab_b = torch.cat((self.x_ab, self.realA_C), 1)
    ...
    def backward_D(self):
        ...
        if not self.opt.use_same_D:
            ...
            # D_loss
            self.loss_D_A, self.loss_gp_A = self.backward_D_basic(self.netD_A, self.real_A, x_ba, self.realB_C)
            self.loss_D_B, self.loss_gp_B = self.backward_D_basic(self.netD_B, self.real_B, x_ab, self.realA_C)
        else:
            x_ba = self.netG_A.decode(self.c_b, s_a)
            x_ab = self.netG_A.decode(self.c_a, s_b)
            # D_loss
            self.loss_D_A, self.loss_gp_A = self.backward_D_basic(self.netD_A, self.real_A, x_ba, self.realB_C)
            self.loss_D_B, self.loss_gp_B = self.backward_D_basic(self.netD_A, self.real_B, x_ab, self.realA_C)
