...
class FTSGINCModel(BaseModel):
    """
    Inspired by MUNIT, enhance regulating two latents
    """
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        #self.netIcp = networks.InceptionV3_feature_extractor('legacy_pytorch', device = self.device)
    ...
    def forward(self):
        ...
        self.c_b_recon_xc2, self.s_a_prime_xcrec, self.condi_xc_xcrec = self.netG_A.encode(self.xc_cbscaitc)
    ...
    def backward_G(self):
        ...
        self.loss_L1_condixc_rec = self.recon_criterion(self.condi_xc, self.condi_xc_xcrec) * self.opt.recon_c_w
        ...
        self.loss_Icp_xa_casaita = self.InceptionLoss(self.netIcp,self.xa_casaita,self.real_A) * self.opt.vgg_w if self.opt.vgg_w > 0 else None
        self.loss_Icp_xa_carasitc = self.InceptionLoss(self.netIcp,self.xa_carasitc,self.real_A) * self.opt.vgg_w if self.opt.vgg_w > 0 else None
        self.loss_Icp_xc_cbscita = self.InceptionLoss(self.netIcp,self.xc_cbscita,self.real_C) * self.opt.vgg_w if self.opt.vgg_w > 0 else None
        self.loss_Icp_xc_cbscaitc = self.InceptionLoss(self.netIcp,self.xc_cbscaitc,self.real_C) * self.opt.vgg_w if self.opt.vgg_w > 0 else None
        self.loss_Icp_xc_cbrscitc = self.InceptionLoss(self.netIcp,self.xc_cbrscitc,self.real_C) * self.opt.vgg_w if self.opt.vgg_w > 0 else None
        ####
        # GAN loss
        ...
        self.loss_Icp_xa_cascita + self.loss_Icp_xa_casaita + self.loss_Icp_xa_carasitc + self.loss_Icp_xc_cbscita +\
        self.loss_Icp_xc_cbscaitc + self.loss_Icp_xc_cbrscitc
