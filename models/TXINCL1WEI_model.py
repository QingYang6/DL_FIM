...
class TXINCL1WEIModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--rs_weight', type=float, default=5)
            parser.add_argument('--crossC_L1_weight', type=float, default=10)
        return parser
    ...
    def __init__(self, opt):
        ...
        'G_xc_ccsbic', 'crossrecon_xacb', 'crossrecon_xbca']
    ...
    def backward_G(self):
        ...
        self.loss_L1_rsa_xarec = self.recon_criterion(self.rsa, self.rsa_xarec) * self.opt.recon_s_w * self.opt.rs_weight
        self.loss_L1_rsa_xcrec = self.recon_criterion(self.rsa, self.rsa_xcrec) * self.opt.recon_s_w * self.opt.rs_weight
        self.loss_L1_rsb_xbrec = self.recon_criterion(self.rsb, self.rsb_xbrec) * self.opt.recon_s_w * self.opt.rs_weight
        self.loss_L1_rsb_xcrec = self.recon_criterion(self.rsb, self.rab_xcrec) * self.opt.recon_s_w * self.opt.rs_weight
        #
        ...
        #
        self.loss_crossrecon_xacb = self.recon_criterion(self.xa_cbsaita, self.real_A) * self.opt.recon_x_cyc_w * self.opt.crossC_L1_weight if self.opt.recon_x_cyc_w > 0 else None
        self.loss_crossrecon_xbca = self.recon_criterion(self.xb_casbitb, self.real_B) * self.opt.recon_x_cyc_w * self.opt.crossC_L1_weight if self.opt.recon_x_cyc_w > 0 else None
        # Inc loss(vgg loss) supplements to GAN loss
        self.loss_Icp_xa_carsacondib = self.InceptionLoss(self.netIcp,self.xa_carsacondib,self.real_A) * self.opt.vgg_w * self.opt.rs_weight if self.opt.vgg_w > 0 else None
        ...
        self.loss_Icp_xb_cbrsbcondia = self.InceptionLoss(self.netIcp,self.xb_cbrsbcondia,self.real_B) * self.opt.vgg_w * self.opt.rs_weight if self.opt.vgg_w > 0 else None
        ...
        self.loss_Icp_xc_ccrsacondic = self.InceptionLoss(self.netIcp,self.xc_ccrsacondic,self.real_C) * self.opt.vgg_w * self.opt.rs_weight if self.opt.vgg_w > 0 else None
        self.loss_Icp_xc_ccrsbcondic = self.InceptionLoss(self.netIcp,self.xc_ccrsbcondic,self.real_C) * self.opt.vgg_w * self.opt.rs_weight if self.opt.vgg_w > 0 else None
        ...
        # GAN loss
        self.loss_G_xa_carsacondib = self.singleGloss(self.netD_A,self.xa_carsacondib,self.real_C_a) * self.opt.rs_weight
        ...
        #
        self.loss_G_xb_cbrsbcondia = self.singleGloss(self.netD_A,self.xb_cbrsbcondia,self.real_C_b) * self.opt.rs_weight
        ...
        #
        self.loss_G_xc_ccrsacondic = self.singleGloss(self.netD_A,self.xc_ccrsacondic,self.real_C_c) * self.opt.rs_weight
        self.loss_G_xc_ccrsbcondic= self.singleGloss(self.netD_A,self.xc_ccrsbcondic,self.real_C_c) * self.opt.rs_weight
        ...
        self.loss_G_xc_ccsbic + self.loss_crossrecon_xacb + self.loss_crossrecon_xbca
