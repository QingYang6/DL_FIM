...
class MUNITUCCLEModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--input_nc_LCC', type=float, default=3)
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'D_B','D_uc_A','D_uc_B','gp_A','gp_B','gp_uc_A','gp_uc_B',\
            ...
            'gen_recon_uc_ba' , 'gen_recon_uc_ab'  \
            , 'genLCCLIC_recon_ca' , 'genLCCLIC_recon_cb' \
            , 'gen_adv_a_uc' , 'gen_adv_b_uc' \
            , 'gen_recon_s_a_ucb' , 'gen_recon_s_b_uca' \
            , 'gen_recon_uca' , 'gen_recon_ucb' ]
    ...
    def forward(self):
        ...
        self.s_c = torch.randn(self.real_B.size(0), self.opt.style_dim, 1, 1).to(self.device)
        self.realA_C = torch.cat((self.real_A, self.real_C_a), 1)
        self.realB_C = torch.cat((self.real_B, self.real_C_b), 1)
        # encode
        self.c_a, self.s_a_prime = self.netG_A.encode(self.realA_C)
        self.c_b, self.s_b_prime = self.netG_A.encode(self.realB_C)
        self.uca, _ = self.netG_LCC.encode(self.real_C_a)
        self.ucb, _ = self.netG_LCC.encode(self.real_C_b)
        # decode (within domain)
        self.x_a_recon = self.netG_A.decode(self.c_a, self.s_a_prime)
        self.x_b_recon = self.netG_A.decode(self.c_b, self.s_b_prime)
        self.xuc_a_recon = self.netG_A.decode(self.uca, self.s_a_prime)
        self.xuc_b_recon = self.netG_A.decode(self.ucb, self.s_b_prime)
        self.LCCLIAa_recon = self.netG_LCC.decode(self.uca)
        self.LCCLIAb_recon = self.netG_LCC.decode(self.ucb)
        # decode (cross domain)
        self.x_ba = self.netG_A.decode(self.c_b, self.s_a)
        self.x_ab = self.netG_A.decode(self.c_a, self.s_b)
        self.x_ucb_a = self.netG_A.decode(self.ucb, self.s_a)
        self.x_uca_b = self.netG_A.decode(self.uca, self.s_b)
        # encode again
        self.x_ba_ca = torch.cat((self.x_ba, self.real_C_a), 1)
        self.x_ab_cb = torch.cat((self.x_ab, self.real_C_b), 1)
        self.x_ucb_a_ca = torch.cat((self.x_ucb_a, self.real_C_a), 1)
        self.x_uca_b_cb = torch.cat((self.x_uca_b, self.real_C_b), 1)
        ...
        self.ucb_recon, self.s_a_ucb_recon = self.netG_A.encode(self.x_ucb_a_ca)
        self.uca_recon, self.s_b_uca_recon = self.netG_A.encode(self.x_uca_b_cb)
    ...
    def backward_G(self):
        ...
        self.loss_genLCCLIC_recon_ca = self.recon_criterion(self.LCCLIAa_recon, self.real_C_a) * self.opt.recon_x_w
        self.loss_genLCCLIC_recon_cb = self.recon_criterion(self.LCCLIAb_recon, self.real_C_b) * self.opt.recon_x_w
        x_uc_ba_a = torch.cat((self.x_ucb_a, self.real_C_b), 1)
        x_uc_ab_b = torch.cat((self.x_uca_b, self.real_C_a), 1)
        ...
        self.loss_gen_recon_s_a_ucb = self.recon_criterion(self.s_a_ucb_recon, self.s_a) * self.opt.recon_s_w
        self.loss_gen_recon_s_b_uca = self.recon_criterion(self.s_b_uca_recon, self.s_b) * self.opt.recon_s_w
        self.loss_gen_recon_uca = self.recon_criterion(self.uca_recon, self.uca) * self.opt.recon_c_w
        self.loss_gen_recon_ucb = self.recon_criterion(self.ucb_recon, self.ucb) * self.opt.recon_c_w
        ...
        self.loss_G = self.loss_gen_recon_x_a + self.loss_gen_recon_x_b + \
            ...
            self.loss_gen_adv_a + self.loss_gen_adv_b + \
                self.loss_gen_vgg_a + self.loss_gen_vgg_b +\
                    self.loss_gen_recon_uc_ba + self.loss_gen_recon_uc_ab + \
                        + self.loss_genLCCLIC_recon_ca + self.loss_genLCCLIC_recon_cb \
                        + self.loss_gen_adv_a_uc + self.loss_gen_adv_b_uc \
                        + self.loss_gen_recon_s_a_ucb + self.loss_gen_recon_s_b_uca \
                        + self.loss_gen_recon_uca + self.loss_gen_recon_ucb
    ...
    def backward_D(self):
        ...
        x_ucb_a = self.netG_A.decode(self.uca, self.s_a)
        x_uca_b = self.netG_A.decode(self.ucb, self.s_b)
        # D_loss
        ...
        self.loss_D_uc_A, self.loss_gp_uc_A = self.backward_D_basic(self.netD_A, self.real_A, x_ucb_a, self.real_C_b)
        self.loss_D_uc_B, self.loss_gp_uc_B = self.backward_D_basic(self.netD_A, self.real_B, x_uca_b, self.real_C_a)
