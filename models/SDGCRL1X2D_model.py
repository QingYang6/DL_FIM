...
class SDGCRL1X2DModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(no_dropout=True,norm='AdaIn', netG='AdaINGen',netD='MsImageDis',\
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A' , 'D_B' , 'D_a2a' , 'D_a2b', 'gp_A' , 'gp_B' , 'gp_a2a' , 'gp_a2b', \
            'D_A_cbra' , 'D_B_carb' , 'gp_A_cbra' , 'gp_B_carb', \
            ...
            'gen_cycrecon_x_a2aa2' , 'gen_cycrecon_x_a2ba2','gen_adv_a2a','gen_adv_a2b','crossxb_casb','crossxa_cbsa']
    ...
    def set_input(self, input):
        ...
        self.real_C2 = raw_BC[:,[3,4],:,:].to(self.device)
        if self.opt.DEM:
            self.real_C_a = raw_AC[:,[2,6,7,8],:,:].to(self.device)
            self.real_C_b = raw_AC[:,[5,6,7,8],:,:].to(self.device)
    ...
    def sample(self):
        ...
        contentC, _ = self.netG_A.encode(self.realA2_C)            
        self.fake_B = self.netG_A.decode(contentA, styleB)
        self.fake_C = self.netG_A.decode(contentC, style_rand)
        self.fake_A = self.netG_A.decode(contentB, styleA)
        self.real_C = self.real_A2
        self.fake_C2 = self.netG_A.decode(contentC, styleB)
    ...
    def forward(self):
        ...
        self.x_a2_recon = self.netG_A.decode(self.c_a2, self.s_a_prime2)
        # decode (cross domain)
        self.x_ba = self.netG_A.decode(self.c_b, self.s_a)
        self.x_ab = self.netG_A.decode(self.c_a, self.s_b)
        self.x_a2a = self.netG_A.decode(self.c_a2, self.s_a_prime)
        self.x_a2b = self.netG_A.decode(self.c_a2, self.s_b_prime)
        self.x_carb = self.netG_A.decode(self.c_a, self.s_b_prime)
        self.x_cbra = self.netG_A.decode(self.c_b, self.s_a_prime)
        # encode again
        if self.opt.CtoE:
        ...
        self.x_a2aa2 = self.netG_A.decode(self.c_a2_recon_a, self.s_a_prime2) if self.opt.recon_x_cyc_w > 0 else None
        self.x_a2ba2 = self.netG_A.decode(self.c_a2_recon_b, self.s_a_prime2) if self.opt.recon_x_cyc_w > 0 else None
    ...
    def backward_G(self):
        ...
        #cross C
        self.loss_crossxb_casb = self.recon_criterion(self.x_carb, self.real_B) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        self.loss_crossxa_cbsa = self.recon_criterion(self.x_cbra, self.real_A) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        #x_a2a_cford = torch.cat((self.x_a2a, self.real_C_a2), 1)
        #x_a2b_cford = torch.cat((self.x_a2b, self.real_C_a2), 1)
        x_a2a_cford = torch.cat((self.x_a2a, self.real_C2), 1)
        x_a2b_cford = torch.cat((self.x_a2b, self.real_C2), 1)
        ...
        self.loss_G = self.loss_gen_recon_x_a + self.loss_gen_recon_x_b + \
            ...
            self.loss_gen_adv_a2a + self.loss_gen_adv_a2b + self.loss_crossxb_casb + self.loss_crossxa_cbsa
    ...
    def backward_D(self):
        ...
        self.loss_D_a2a, self.loss_gp_a2a = self.backward_D_basic(self.netD_A, self.real_A2, self.x_a2a.detach(), self.real_C2)
        self.loss_D_a2b, self.loss_gp_a2b = self.backward_D_basic(self.netD_A, self.real_A2, self.x_a2b.detach(), self.real_C2)
        self.loss_D_A_cbra, self.loss_gp_A_cbra = self.backward_D_basic(self.netD_A, self.real_A, self.x_cbra.detach(), self.real_B)
        self.loss_D_B_carb, self.loss_gp_B_carb = self.backward_D_basic(self.netD_A, self.real_B, self.x_carb.detach(), self.real_A)
        self.loss_D = self.loss_D_A + self.loss_D_B + self.loss_D_a2a + self.loss_D_a2b + self.loss_D_A_cbra + self.loss_D_B_carb
        self.loss_gp = self.loss_gp_A + self.loss_gp_B + self.loss_gp_a2a + self.loss_gp_a2b + self.loss_gp_A_cbra + self.loss_gp_B_carb
