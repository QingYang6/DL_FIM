...
class IGARSSNORSModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.loss_names =['D_xa', 'D_xb','gp_xa' ,'gp_xb',\
            'gen_recon_x_a' , 'gen_recon_x_b' , 'gen_recon_s_a' , 'gen_recon_s_b' , \
                'gen_recon_c_ra' , 'gen_recon_c_rb' , 'gen_cycrecon_x_ra' , 'gen_cycrecon_x_rb' , \
                'gen_cross_similar_xb' , 'gen_cross_similar_xa' , 'gen_adv_rsa_xa' , 'gen_adv_rsa_xb']
    ...
    def sample(self):
        ...
        contentB, styleB = self.netG_A.encode(self.realB_C) 
        contentC, styleC = self.netG_A.encode(self.realC_C)             
        self.fake_B = self.netG_A.decode(contentB, styleA, condi_b)
        #self.fake_C = self.netG_A.decode(contentC, style_rand, condi_c)
        self.fake_A = self.netG_A.decode(contentA, styleB, condi_a)
        #self.fake_C2 = self.netG_A.decode(contentC, styleB, condi_c)
        self.real_C = self.netG_A.decode(contentA, styleC, condi_a)
        self.fake_C = self.netG_A.decode(contentB, styleC, condi_b)
        self.fake_C2 = self.netG_A.decode(contentC, styleA, condi_c)
    ...
    def forward(self):
        ...
        self.xa_cbraitb = self.netG_A.decode(self.c_b, self.s_a_prime, self.ita)
        self.xb_carbita = self.netG_A.decode(self.c_a, self.s_b_prime, self.itb)
        # encode again
        if self.opt.CtoE:
            self.xa_recon_cbraitb = torch.cat((self.xa_cbraitb, self.real_C_a), 1)
            self.xb_recon_carbita = torch.cat((self.xb_carbita, self.real_C_b), 1)
        else:
            self.xc_ccitcsa_condic = self.xc_ccitcsa         
        self.c_b_recon, self.s_a_recon = self.netG_A.encode(self.xa_recon_cbraitb)
        self.c_a_recon, self.s_b_recon = self.netG_A.encode(self.xb_recon_carbita)
        # decode again (if needed)
        self.x_a_blue = self.netG_A.decode(self.c_a_recon, self.s_a_prime, self.ita) if self.opt.recon_x_cyc_w > 0 else None
        self.x_b_blue = self.netG_A.decode(self.c_b_recon, self.s_b_prime, self.itb) if self.opt.recon_x_cyc_w > 0 else None
    ...
    def backward_G(self):
        ...
        self.loss_gen_recon_s_a = self.recon_criterion(self.s_a_recon, self.s_a_prime) * self.opt.recon_s_w
        self.loss_gen_recon_s_b = self.recon_criterion(self.s_b_recon, self.s_b_prime) * self.opt.recon_s_w
        self.loss_gen_recon_c_ra = self.recon_criterion(self.c_a_recon, self.c_a) * self.opt.recon_c_w
        self.loss_gen_recon_c_rb = self.recon_criterion(self.c_b_recon, self.c_b) * self.opt.recon_c_w
        #reconstruction loss
        self.loss_gen_cycrecon_x_ra = self.recon_criterion(self.x_a_blue, self.real_A) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        self.loss_gen_cycrecon_x_rb = self.recon_criterion(self.x_b_blue, self.real_B) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        #
        self.loss_gen_cross_similar_xa = self.recon_criterion(self.xa_cbraitb, self.real_A) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        self.loss_gen_cross_similar_xb = self.recon_criterion(self.xb_carbita, self.real_B) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        # GAN loss
        rsa_xa = torch.cat((self.xa_cbraitb, self.real_B), 1)
        rsa_xb = torch.cat((self.xb_carbita, self.real_A), 1)
        ...
        self.loss_G = self.loss_gen_recon_x_a + self.loss_gen_recon_x_b + self.loss_gen_recon_s_a + self.loss_gen_recon_s_b + \
                self.loss_gen_recon_c_ra + self.loss_gen_recon_c_rb + self.loss_gen_cycrecon_x_ra + self.loss_gen_cycrecon_x_rb + \
                self.loss_gen_cross_similar_xb + self.loss_gen_cross_similar_xa + self.loss_gen_adv_rsa_xa + self.loss_gen_adv_rsa_xb
    ...
    def backward_D(self):
        ...
        self.loss_D_xa, self.loss_gp_xa = self.backward_D_basic(self.netD_A, self.real_A, self.xa_cbraitb, self.real_B)
        self.loss_D_xb, self.loss_gp_xb = self.backward_D_basic(self.netD_A, self.real_B, self.xb_carbita, self.real_A)
