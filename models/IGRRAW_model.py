...
class IGARSSSMALLModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_xa', 'D_xb','gp_xa' ,'gp_xb',\
            ...
            'gen_recon_xa' , 'gen_recon_xb' , 'gen_recon_ca' , 'gen_recon_cb' , \
        'gen_adv_rsa_xa' , 'gen_adv_rsa_xb' ]
    ...
    def sample(self):
        ...
        style_rand2 = torch.randn(self.real_A.size(0), self.opt.style_dim).to(self.device)
        if self.opt.CtoE:
            self.realA_C = torch.cat((self.real_A, self.real_C_a), 1)
            self.realB_C = torch.cat((self.real_B, self.real_C_b), 1)
            self.realC_C = torch.cat((self.real_C, self.real_C_c), 1)
        ...
        self.fake_B = self.netG_A.decode(contentA, style_rand2, condi_b)
        #self.fake_C = self.netG_A.decode(contentC, style_rand, condi_c)
        self.fake_A = self.netG_A.decode(contentB, style_rand2, condi_a)
    ...
    def backward_G(self):
        ...
        #reconstruction loss
        self.loss_gen_recon_xa = self.recon_criterion(self.xa_recon, self.real_A) * self.opt.recon_x_w
        self.loss_gen_recon_xb = self.recon_criterion(self.xb_recon, self.real_B) * self.opt.recon_x_w
        #
        ...
        self.loss_G = self.loss_gen_cycrecon_rsaxaxb + \
            self.loss_gen_recon_xa + self.loss_gen_recon_xb + self.loss_gen_recon_ca + self.loss_gen_recon_cb + \
            self.loss_gen_adv_rsa_xa + self.loss_gen_adv_rsa_xb 
