...
class DecomposeAttnModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        visual_names_A = ['real_A', 'fake_B', 'rec_A', 'fake_B_re', 'att_A2B_viz']
        visual_names_B = ['real_B', 'fake_A', 'rec_B', 'fake_A_re', 'att_B2A_viz']
    ...
    def forward(self):
        # Regular Feed Forward Phase - First pass, no attn map
        ...
        self.att_A2B_viz, self.att_B2A_viz = (attn_fake_B - 0.5) / 0.5, (attn_fake_A - 0.5) / 0.5

        self.rec_A = self.netG_B(self.fake_B)  # G_B(G_A(A))
        self.rec_B = self.netG_A(self.fake_A)  # G_A(G_B(B))

        # Additional Feed Forward using attention information - Second pass
        if self.concat == 'mult':
            self.fake_B_re = self.netG_A(self.real_A * attn_fake_B.detach())
            self.fake_A_re = self.netG_B(self.real_B * attn_fake_A.detach())
        elif self.concat == 'rmult':
            self.fake_B_re = self.netG_A(self.real_A * (1. + attn_fake_B.detach()))
            self.fake_A_re = self.netG_B(self.real_B * (1. + attn_fake_A.detach()))
        else:
            raise NotImplementedError('Unsupported concatenation method')

        self.rec_A_re = self.netG_B(self.fake_B_re)
        self.rec_B_re = self.netG_A(self.fake_A_re)
    ...
    def backward_G(self):
        ...
        # GAN & Cycle Loss for 2nd Pass
        dis_A_re_res, _ = self.netD_A(self.fake_B_re)
        self.loss_G_A_2 = self.criterionGAN(dis_A_re_res, True)

        dis_B_re_res, _ = self.netD_B(self.fake_A_re)
        self.loss_G_B_2 = self.criterionGAN(dis_B_re_res, True)

        self.loss_cycle_A_2 = self.criterionCycle(self.rec_A_re, self.real_A) * lambda_A
        self.loss_cycle_B_2 = self.criterionCycle(self.rec_B_re, self.real_B) * lambda_B
        # self.loss_cycle_A_2 = 0.
        # self.loss_cycle_B_2 = 0.

        # Combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + \
                      self.loss_G_A_2 + self.loss_G_B_2 + \
                      self.loss_cycle_A + self.loss_cycle_B + \
                      self.loss_cycle_A_2 + self.loss_cycle_B_2 + \
