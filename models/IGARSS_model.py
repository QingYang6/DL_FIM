...
class IGARSSModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        parser.set_defaults(no_dropout=True,norm='adain', netG='AdaINGen_IA',netD='MsImageDis',\
            use_same_D=True, use_same_G=True, CtoE = True, DEM = True)  # default MUNIT did not use dropout
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_xagold' , 'D_xcpurple', \
        'gp_xagold' , 'gp_xcpurple', \
            'gen_recon_xa_red' , 'gen_recon_xb_blue' , \
            'gen_recon_sb_blue' , 'gen_recon_rsa_gold' , 'gen_recon_sc_purple' \
                , 'gen_recon_ca_blue', \
            'gen_recon_ca_gold' , 'gen_recon_cc_purple' ,\
            'gen_cycrecon_xa_blue' , 'gen_cycrecon_xa_gold' , \
                'gen_cycrecon_xc_purple' , 'gen_adv_xagold' , \
            'gen_adv_xcpurple' , 'gen_vgg_a' , 'gen_vgg_b']
    ...
    def set_input(self, input):
        ...
        self.real_D = raw_BC[:,[3,4],:,:].to(self.device)
        if self.opt.DEM:
            self.real_C_a = raw_AC[:,[2,6,7,8],:,:].to(self.device)
            self.real_C_b = raw_AC[:,[5,6,7,8],:,:].to(self.device)
    ...
    def forward(self):
        self.s_a = torch.randn(self.real_A.size(0), self.opt.style_dim).to(self.device)
        ...
        self.xa_recon = self.netG_A.decode(self.c_a, self.s_a_prime, self.ita)
        #self.xb_recon = self.netG_A.decode(self.c_b, self.s_b_prime, self.itb)
        #self.xc_recon = self.netG_A.decode(self.c_c, self.s_c_prime, self.itc)
        # decode (cross domain)
        self.xa_caitbsb = self.netG_A.decode(self.c_a, self.s_b_prime, self.itb)
        self.xa_caitarsa = self.netG_A.decode(self.c_a, self.s_a, self.ita)
        self.xc_ccitcsa = self.netG_A.decode(self.c_c, self.s_a_prime, self.itc)
        # encode again
        if self.opt.CtoE:
            #self.xa_recon_condia = torch.cat((self.xa_recon, self.real_C_a), 1)
            self.xa_caitbsb_condib = torch.cat((self.xa_caitbsb, self.real_C_b), 1)
            self.xa_caitarsa_condia = torch.cat((self.xa_caitarsa, self.real_C_a), 1)
            self.xc_ccitcsa_condic = torch.cat((self.xc_ccitcsa, self.real_C_c), 1)
        else:
            #self.xa_recon_condia = self.xa_recon
            self.xa_caitbsb_condib = self.xa_caitbsb
            self.xa_caitarsa_condia = self.xa_caitarsa
            self.xc_ccitcsa_condic = self.xc_ccitcsa         
        self.c_a_recon_blue, self.s_b_recon_blue = self.netG_A.encode(self.xa_caitbsb_condib)
        self.c_a_recon_gold, self.rsa_recon_gold = self.netG_A.encode(self.xa_caitarsa_condia)
        self.c_c_recon_purple, self.s_a_recon_purple = self.netG_A.encode(self.xc_ccitcsa_condic)
        # decode again (if needed)
        self.x_a_blue = self.netG_A.decode(self.c_a_recon_blue, self.s_a_prime, self.ita) if self.opt.recon_x_cyc_w > 0 else None
        self.x_a_gold = self.netG_A.decode(self.c_a_recon_gold, self.s_a_prime, self.ita) if self.opt.recon_x_cyc_w > 0 else None
        self.x_c_purple = self.netG_A.decode(self.c_c_recon_purple, self.s_c_prime, self.itc) if self.opt.recon_x_cyc_w > 0 else None

    def backward_G(self):
        """Calculate the loss for generators G_A and G_B"""
        #reconstruction loss
        self.loss_gen_recon_xa_red = self.recon_criterion(self.xa_recon, self.real_A) * self.opt.recon_x_w
        self.loss_gen_recon_xb_blue = self.recon_criterion(self.xa_caitbsb, self.real_B) * self.opt.recon_x_w
        #
        self.loss_gen_recon_sb_blue = self.recon_criterion(self.s_b_recon_blue, self.s_b_prime) * self.opt.recon_s_w
        self.loss_gen_recon_rsa_gold = self.recon_criterion(self.rsa_recon_gold, self.s_a) * self.opt.recon_s_w
        self.loss_gen_recon_sc_purple = self.recon_criterion(self.s_a_recon_purple, self.s_a_prime) * self.opt.recon_s_w
        #
        self.loss_gen_recon_ca_blue = self.recon_criterion(self.c_a_recon_blue, self.c_a) * self.opt.recon_c_w
        self.loss_gen_recon_ca_gold = self.recon_criterion(self.c_a_recon_gold, self.c_a) * self.opt.recon_c_w
        self.loss_gen_recon_cc_purple = self.recon_criterion(self.c_c_recon_purple, self.c_c) * self.opt.recon_c_w
        #
        self.loss_gen_cycrecon_xa_blue = self.recon_criterion(self.x_a_blue, self.real_A) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        self.loss_gen_cycrecon_xa_gold = self.recon_criterion(self.x_a_gold, self.real_A) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        self.loss_gen_cycrecon_xc_purple = self.recon_criterion(self.x_a_gold, self.real_A) * self.opt.recon_x_cyc_w if self.opt.recon_x_cyc_w > 0 else None
        # GAN loss
        xa_gold_comb = torch.cat((self.xa_caitarsa, self.real_B), 1)
        xc_purple_comb = torch.cat((self.xc_ccitcsa, self.real_D), 1)
        self.loss_gen_adv_xagold = self.criterionGAN(self.netD_A(xa_gold_comb),True) * self.opt.gan_w
        self.loss_gen_adv_xcpurple = self.criterionGAN(self.netD_A(xc_purple_comb),True) * self.opt.gan_w
        #
        ...
        self.loss_G = self.loss_gen_recon_xa_red + self.loss_gen_recon_xb_blue + \
            self.loss_gen_recon_sb_blue + self.loss_gen_recon_rsa_gold + self.loss_gen_recon_sc_purple \
                + self.loss_gen_recon_ca_blue+ \
            self.loss_gen_recon_ca_gold + self.loss_gen_recon_cc_purple +\
            self.loss_gen_cycrecon_xa_blue + self.loss_gen_cycrecon_xa_gold + \
                self.loss_gen_cycrecon_xc_purple + self.loss_gen_adv_xagold + \
            self.loss_gen_adv_xcpurple + self.loss_gen_vgg_a + self.loss_gen_vgg_b
    ...
    def backward_D(self):
        ...
        # decode (cross domain)
        x_ba = self.netG_A.decode(self.c_b, s_a, self.ita)
        #x_ab = self.netG_A.decode(self.c_a, s_b)
        # D_loss
        self.loss_D_xagold, self.loss_gp_xagold = self.backward_D_basic(self.netD_A, self.real_A, x_ba, self.real_B)
        self.loss_D_xcpurple, self.loss_gp_xcpurple = self.backward_D_basic(self.netD_A, self.real_C, self.xc_ccitcsa, self.real_D)
        self.loss_D = self.loss_D_xagold + self.loss_D_xcpurple
        self.loss_gp = self.loss_gp_xagold + self.loss_gp_xcpurple
